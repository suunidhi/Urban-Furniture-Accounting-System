...
const PurchasesList = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      setErrorMsg(null);
      const [purRes, conRes, prodRes, anaRes] = await Promise.all([
        api.get('/purchases'),
        api.get('/contacts'),
        api.get('/products'),
      ...
      setVendors(conRes.data.filter(c => c.type === 'vendor' || c.type === 'both'));
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!vendorId) return setErrorMsg('Please select a vendor');
    if (lines.length === 0) return setErrorMsg('Please add at least one product');
  ...
  const confirmOrder = async (id) => {
    try {
      ...
      console.error(err);
      const backendMessage = err.response?.data?.message || 'Failed to confirm purchase order';
    ...
    { 
      ...
      render: (row) => (
        <span className={`px-2 py-1 rounded text-xs font-semibold capitalize ${
          ...
          row.status === 'billed' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100'
        }`}>
          {row.status}
        </span>
      )
          ...
          {row.status === 'confirmed' && (
            <button 
              onClick={async (e) => { 
                ...
                try {
                  setErrorMsg(null);
                  await api.post(`/bills/from-po/${row.id}`);
                  fetchData();
                } catch(err) {
                  const backendMessage = err.response?.data?.message || 'Failed to create bill';
                  setErrorMsg(backendMessage);
                }
              }} 
    ...
    return (
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-light text-odoo-primary">New Purchase Order</h2>
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-light text-odoo-primary">Purchase Orders</h2>
        <button 
          onClick={() => {
            setVendorId('');
            setLines([]);
