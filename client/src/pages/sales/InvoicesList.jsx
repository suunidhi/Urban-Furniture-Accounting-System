...
const InvoicesList = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error(err);
      setErrorMsg('Failed to fetch invoices');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmInvoice = async (id) => {
    try {
      ...
      console.error(err);
      const backendMessage = err.response?.data?.message || 'Failed to confirm invoice';
  ...
  const openReport = async (id) => {
    try {
      ...
      console.error(err);
      setErrorMsg('Failed to load invoice details for report');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex gap-2">
          {row.status === 'draft' && (
            <button 
              onClick={(e) => { e.stopPropagation(); confirmInvoice(row.id); }} 
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-light text-odoo-primary">Customer Invoices</h2>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center print:hidden">
          <div className="relative p-5 border w-[32rem] shadow-lg rounded-xl bg-white">
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-6 flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-500 font-medium uppercase tracking-wide">Customer</p>
                <p className="text-gray-900 font-semibold">{selectedRow.customer ? selectedRow.customer.name : 'Unknown'}</p>
              </div>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center print:static print:bg-white print:h-auto print:w-auto print:block">
          <div className="relative p-10 border w-[48rem] shadow-2xl rounded-xl bg-white print:w-full print:border-none print:shadow-none print:p-0">
            ...
            <div className="flex justify-between items-center mb-8 pb-4 border-b border-gray-200 print:hidden">
              <h3 className="text-2xl font-light text-gray-800">Customer Invoice Report</h3>
            ...
            <div className="print-section">
              <div className="flex justify-between items-start mb-10">
                <div>
                  <h1 className="text-4xl font-bold text-odoo-primary mb-2">{reportData.invoice_number}</h1>
                ...
                <div className="text-right text-gray-600">
                  <p className="font-bold text-gray-800 text-lg mb-1">{reportData.customer?.name}</p>
                  <p className="text-sm">Customer ID: {reportData.customer?.id}</p>
                </div>
              </div>
                ...
                <tbody>
                  {reportData.lines && reportData.lines.map((line, idx) => (
                    <tr key={idx} className="border-b border-gray-200">
                      ...
                      <td className="py-3 px-2 text-gray-500 border-r border-gray-200 text-sm">Sales a/c</td>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {invoices.map(invoice => (
              <div key={invoice.id} onClick={() => openReport(invoice.id)} className="bg-white rounded border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-4 cursor-pointer relative">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${
                    ...
                    invoice.status === 'posted' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                  }`}>
                    {invoice.status}
                  </span>
                </div>
                <div className="mb-3">
                  <p className="text-sm font-medium text-gray-700">Customer: {invoice.customer ? invoice.customer.name : '-'}</p>
                </div>
                <div className="flex justify-between items-end border-t border-gray-100 pt-3">
                  <div>
                    <p className="text-xs text-gray-500 mb-1">Amount Due</p>
                    <p className="font-bold text-odoo-primary">${Number(invoice.outstanding_amount).toFixed(2)}</p>
                  </div>
                  <div className="flex gap-2">
                    {invoice.status === 'draft' && (
                      <button 
                        onClick={(e) => { e.stopPropagation(); confirmInvoice(invoice.id); }} 
