...
export const CustomerAnalysisPage: React.FC = () => {
  ...
  const { data: analysisData, isLoading } = useQuery({
    queryKey: ['customer-analysis', timeRange, type],
    queryFn: async () => {
      const res = await api.get('/analysis/customer', {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Users className="w-7 h-7 text-[#714B67]" />
            Customer Analysis
          </h1>
          <p className="text-gray-500 text-sm mt-1">Analyze revenue distribution across customers</p>
        </div>
      </div>
